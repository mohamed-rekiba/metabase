...
import { oidcProvisioningOptions } from "metabase/admin/settings/auth/utils/provisioningOptions";
import {
...
import {
  ...
  type OidcConnectionCheckRequest,
  type OidcProviderConfig,
} from "metabase/api/sso-oidc";
...
import {
  formValuesToProvider,
  oidcFormSchema,
  providerToFormValues,
  type OidcFormValues,
} from "./oidc-form-utils";

export function SettingsOidcForm() {
  const applicationName = useSelector(getApplicationName);
  const siteUrl = useSetting("site-url");
  const { data: providers, isLoading } = useListOidcProvidersQuery();
  ...
  const runConnectionCheck = useCallback(
    async (values: OidcFormValues) => {
      const request: OidcConnectionCheckRequest = {
        "issuer-uri": values["issuer-uri"],
        "client-id": values["client-id"],
      };

      if (values["client-secret"]) {
        request["client-secret"] = values["client-secret"];
      } else if (isExisting && existingProvider) {
        request.key = existingProvider.key;
      }

      return await checkConnection(request).unwrap();
    },
  ...
  const handleCheckConnection = useCallback(
    async (values: OidcFormValues) => {
      try {
        const result = await runConnectionCheck(values);
    ...
    [runConnectionCheck, sendToast],
  );

  const handleSubmit = useCallback(
    async (values: OidcFormValues) => {
      await runConnectionCheck(values);
      ...
      <Stack gap="lg">
        <SettingsSection>
          <AdminSettingInput
            ...
            inputType="radio"
            options={oidcProvisioningOptions("OIDC")}
          />
        </SettingsSection>
        ...
        <FormProvider
          ...
          validationSchema={oidcFormSchema()}
          enableReinitialize
        >
          {({ dirty, values }) => (
            <Form>
                ...
                <FormSection title={t`Server settings`}>
                  <Stack gap="md">
                    ...
                    <FormTextInput
                      ...
                      placeholder={t`e.g. Sign in with Keycloak`}
                      required
                    />
                    <FormTextInput
                      name="issuer-uri"
